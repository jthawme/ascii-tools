...
export class AsciiGrid extends AsciiGraphical {
  ...
  get cellWidth() {
    return Math.floor((this.width - 1) / this.cols); // minus 1 because of the last border
  }
  ...
  boxFactory(x, y, opts) {
    const box = new AsciiBox(
      this.cellWidth + (x === this.cols - 1 ? 1 : 0),
      this.cellHeight,
      {
        ...opts,
        graphics: {
    ...
    box.move(x * this.cellWidth, y * (this.cellHeight - 1));
